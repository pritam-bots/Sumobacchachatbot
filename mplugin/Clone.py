...
import config
from ROCKY.mplugin.helpers import is_owner
from config import API_HASH, API_ID, OWNER_ID
from ROCKY import CLONE_OWNERS
from ROCKY import ROCKY as app, save_clonebot_owner
from ROCKY import db as mongodb, ROCKY
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="ROCKY/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
        ...
        try:
            ...
            await mi.edit_text(f"**Your bot successfully cloned👍**\n**You can check by /cloned**\n\n**But please start me (@{ROCKY.username}) From owner id**")
        
        except BaseException as e:
            logging.exception("Error while cloning bot.")
            await mi.edit_text(
                f"⚠️ <b>Error:</b>\n\n<code>{e}</code>\n\n**Forward this message to @RockyXSupport for assistance**"
            )
    else:
