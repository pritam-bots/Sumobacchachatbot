...
from ROCKY import db as mongodb
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ...
            await mi.edit_text(
                f"⚠️ <b>Error:</b>\n\n<code>{e}</code>\n\n**Forward this message to @ROCKYXUPDATE for assistance**"
            )
    else:
...
async def restart_bots():
    ...
    try:
        ...
        async def restart_bot(bot):
            bot_token = bot["token"]
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="ROCKYmplugin"))
            try:
                await ai.start()
